...
var getOrders = function(context) {
    ...
    if ( otherPlanets != null && otherPlanets.length > 0 )
    {
        for ( var i = 0; i<myPlanets.length; i++ )
        {
            ...
            var pendingPopulation = myPlanet.population;
            var nearestPlanet     = getNearestPlanet(myPlanet,otherPlanets);

            for(var j = 0; j < otherPlanets.length; j ++){
                var scopedPlanet = otherPlanets[j],
                    sendOut = Math.ceil(scopedPlanet.population/1.5);
                /* debugMessage+="sending out "+sendOut+"\n"; */
                if((pendingPopulation - sendOut) > 0){
                    if(myPlanet.population >= 30){
                        result.push( new Order( myPlanet.id, scopedPlanet.id, sendOut ) );
                    }
                }
                pendingPopulation -= sendOut;
            }
        }
...
var populateNearestPlanet = function(myPlanet,otherPlanets){

    var nearestPlanet = getNearestPlanet(myPlanet,otherPlanets);
    return new Order(myPlanet,nearestPlanet.id,nearestPlanet.population);
}


var getUsersHomePlanet = function(source, candidats){
    var result = candidats[ 0 ];
    var currentDist = GameUtil.getDistanceBetween( new Point( source.x, source.y ), new Point( result.x, result.y ) );
    for ( var i = 0; i<candidats.length; i++ )
    {
