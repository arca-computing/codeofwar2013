...
var firstEPlanet = null;

var findTarget = function(p1, p2) {
...
var getOrders = function(context) {
  ...
  if(firstPlanet == null) {
    firstPlanet = myPlanets[0].id;
    firstEPlanet = otherPlanets[0].id;
  }
	...
	if ( otherPlanets != null && otherPlanets.length > 0 )
  {
    ...
    for ( var i = 0; i<myPlanets.length; i++ )
    {
      ...
      if(myPlanet.population >= 20 || ((myPlanet.id == firstPlanet) && (myPlanets.population >= 10))) 
      {
        force += myPlanet.population;
      }
    }

    for ( var i = 0; i<otherPlanets.length; i++ )
    {
      otherPlanets[i].population += 50;
    }

    var target = otherPlanets;
    target.sort(findTarget);

    var next = target.pop()
    while((force > 0) && (force >= next.population))
    {
      var t = next;
      for ( var i = 0; i<myPlanets.length; i++ )
      {
        ...
        if(myPlanet.population >=20 || ((myPlanet.id == firstPlanet) && (myPlanets.population >= 10))) 
        {
          if(t.population > myPlanet.population)
          {
            result.push( new Order( myPlanet.id, t.id, myPlanet.population ) );
            t.population -= myPlanet.population;
            force -= myPlanet.population;
            myPlanet.population = 0;
          }
          else {
            result.push( new Order( myPlanet.id, t.id, t.population ) );
            myPlanet.population -= t.population;
            force -= t.population;
            t.population = 0;
          }
        }
